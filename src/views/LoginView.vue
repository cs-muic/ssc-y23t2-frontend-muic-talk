        ...
        <div class="col-sm-12 col-md-4 my-auto">
          <div class="form-title">
            <div class="webapp-logo">
              <img src="../main/webapp/Images/MuicTALK_Logo.png" alt="Webapp Logo" />
            </div>
            <h2>Login</h2>
          </div>
          ...
          <v-form ref="form" v-model="valid" lazy-validation>
            ...
            <v-text-field type="password" v-model="password.password" :rules="passwordRules" label="Password" required></v-text-field>

            <v-btn :disabled="!valid" color="primary" class="btn btn-primary" @click="submit_login">Login</v-btn>
          </v-form>
          <!-- Link to Sign Up -->
          <div class="mt-3">
            Don't have an account? <span class="link" @click="show = !show">Sign Up</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="container login-container" v-else>
      <div class="row justify-content-md-center h-100">
        <div class="col-sm-12 col-md-4 my-auto">
          ...
          <div class="alert alert-danger" role="alert" v-show="signUpError.show">
            Failed to Sign Up: {{ signUpError.message }}
          </div>
          <v-form ref="form" v-model="valid" lazy-validation>
            <v-text-field v-model="username" :rules="usernameRules" label="Username" required></v-text-field>
            <v-text-field v-model="displayName" :rules="displayNameRules" label="Display Name" required></v-text-field>
            <v-text-field type="password" v-model="password.password" :rules="passwordRules" label="Password" required></v-text-field>
            <v-btn :disabled="!valid" color="primary" class="btn btn-primary" @click="submit_signup">Sign Up</v-btn>
          </v-form>
          <!-- Link to Login -->
          <div class="mt-3">
            Already have an account? <span class="link" @click="show = !show">Login</span>
          </div>
        </div>
      </div>
    </div>
...
.login-container {
  background-color: rgba(255, 255, 255, 0.7); /* Adjust the opacity value as needed */
  padding: 20px;
  border-radius: 10px;
}
</style>